...
const FlightResults = ({ onBookFlight }) => {
  ...
  // Function to apply 20% markup to price
  const applyMarkup = (originalPrice) => {
    const price = parseFloat(originalPrice) || 0;
    return Math.round(price * 1.2); // 20% markup, rounded to nearest dollar
  };

  // Function to get display price with markup
  const getDisplayPrice = (flight) => {
    return applyMarkup(flight.originalPrice || flight.price || 0);
  };

  // Function to get original price for calculations
  const getOriginalPrice = (flight) => {
    return parseFloat(flight.originalPrice || flight.price || 0);
  };
  ...
  const performFlightSearch = async () => {
    ...
    try {
      ...
      const results = await response.json();
      // Store original prices before applying markup
      const flightsWithOriginalPrice = (results.flights || results || []).map(flight => ({
        ...flight,
        originalPrice: flight.price, // Store original price
        price: applyMarkup(flight.price) // Apply markup to display price
      }));
      
      setSearchResults(flightsWithOriginalPrice);
    } catch (error) {
      console.error('Flight search error:', error);
      setError('Failed to load flight results. Please try again.');
    } finally {
  ...
  // Sort flights based on selected criteria (using display prices)
  const sortedFlights = [...searchResults].sort((a, b) => {
    switch (sortBy) {
      case 'price':
                  ...
                  >
                    <div className="p-6">
                      ...
                      <div className="flex items-center justify-between mb-4">
                        ...
                        <div className="text-right">
                          <div className="text-2xl font-bold text-gray-800">${getDisplayPrice(flight)}</div>
                          <div className="text-sm text-gray-500">per person</div>
                        </div>
                      </div>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                              ...
                              <div className="space-y-2 text-sm">
                                <div className="flex justify-between">
                                  <span className="text-gray-600">Base Fare:</span>
                                  <span className="font-medium">${getDisplayPrice(flight) - 50}</span>
                                </div>
                                <div className="flex justify-between">
                                  <span className="text-gray-600">Taxes & Fees:</span>
                                  <span className="font-medium">$50</span>
                                ...
                                <div className="flex justify-between border-t pt-2 font-semibold">
                                  <span>Total per person:</span>
                                  <span>${getDisplayPrice(flight)}</span>
                                </div>
                                <div className="flex justify-between text-lg font-bold text-blue-600">
                                  <span>Total for {totalPassengers} passengers:</span>
                                  <span>${getDisplayPrice(flight) * totalPassengers}</span>
                                </div>
                              </div>
                            </div>
                          </div>
