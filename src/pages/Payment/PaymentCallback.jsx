...
const PaymentCallback = () => {
  ...
  const [isVerifying, setIsVerifying] = useState(true);
  const [verificationResult, setVerificationResult] = useState(null);
  const [paymentType, setPaymentType] = useState('booking'); // 'booking' or 'wallet'
  ...
  useEffect(() => {
    const verifyPayment = async () => {
      try {
        ...
        // Determine payment type based on tx_ref
        const isWalletFunding = txRef.includes('wallet_fund_');
        setPaymentType(isWalletFunding ? 'wallet' : 'booking');

        // Use transaction_id if available, otherwise use tx_ref
        const verificationId = transactionId || txRef;

        // ✅ FIXED: All wallet funding should go through payments endpoint
        // because that's where the verifyWalletFunding logic is implemented
        const verificationEndpoint = `/api/payments/verify/${verificationId}`;

        // Verify payment on backend
        const token = localStorage.getItem('token');
        const response = await fetch(`${backendUrl}${verificationEndpoint}`, {
          method: 'GET',
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json'
        ...
        // Handle verification result based on payment type
        if (result.success && result.data?.successful) {
          if (isWalletFunding) {
          ...
          // Redirect based on payment type after delay
          setTimeout(() => {
            if (isWalletFunding) {
              navigate('/wallet', { replace: true });
            } else {
        ...
        // Redirect after error based on payment type
        setTimeout(() => {
    ...
    verifyPayment();
  }, [searchParams, navigate, paymentType]);
  ...
  const getPageTitle = () => {
    if (isVerifying) {
      return paymentType === 'wallet' ? 'Processing Wallet Funding' : 'Processing Payment';
    }
    return paymentType === 'wallet' ? 'Wallet Funding Status' : 'Payment Status';
  };

  const getRedirectButtonText = () => {
    return paymentType === 'wallet' ? 'View My Wallet' : 'View My Bookings';
  };

  const getRedirectPath = () => {
    return paymentType === 'wallet' ? '/wallet' : '/bookings';
  };
          ...
          {verificationResult && (
            <div className="bg-gray-50 rounded-lg p-4 mb-6 text-left">
              ...
              <div className="text-sm text-gray-600 space-y-1">
                ...
                {paymentType === 'wallet' && verificationResult.data?.newBalance && (
                  <p><span className="font-medium">New Wallet Balance:</span> ₦{verificationResult.data.newBalance.toLocaleString()}</p>
                )}
              </div>
            </div>
          )}
